--	中兴-控制器的协议(V3.00).doc
--9600或19200或19200或2400B/S 等，SM内应有改变速率的机制。 
--数据格式：1起始位，8数据位，1停止位，校验方式位。 校验方式有: 无校验; 奇校验; 偶校验;  MARK(1)校验;  SPACE(0)校验.	
...
local rs = 
	{
		-- step1, 
		{
			pack = l_gb.pack({0xF2, 0xE0, 0x00}, g_ver, g_adr, 0x80, 0x4A),
			parse = 
				{
		...
		-- step2, 
		{
			pack = l_gb.pack({0xF2, 0xE7, 0x00}, g_ver, g_adr, 0x80, 0x4A),
			parse = 
				{
					{"BYTE", "J6"},
					{"BIT", "B0", 17003001},
...
function parse(c_rs)
	...
	--F1 E0 20 16 02 25 01 17 48 50
		if #packInfo == 14 then	
			local secFrom1970 = string.format("%02X", packInfo[7]) .. string.format("%02X", packInfo[8]) .. "-" 
			secFrom1970 = secFrom1970 .. string.format("%02X", packInfo[9]) .. "-"
			secFrom1970 = secFrom1970 .. string.format("%02X", packInfo[10]) .. " "
...
--	g_setCmd = {}
	...
	for idx = 1, count do
		...
		l_base.printStr("===================== DOOR_ZTE CMD SET:[" .. cmd[1] .. "]=" .. cmd[2] .. "=====================")
		...
		elseif(cmd[1] == 17202001) then
			...
			l_base.printStr("=====================UNKOWN DOOR_ZTE CMD SET:[" .. cmd[1] .. "]=" .. cmd[2] .. "=====================")
		end
	end
end
...
function getSyncTimeCmd(value)
	...
	local rs = 
	{
		pack = l_gb.pack({0xF1, 0xE0}, g_ver, g_adr, 0x80, 0x49),
		parse = 
			{
				{"BYTE", "J3"},
				{"BYTE", "U8", 17201001, "rs == 0"},
